						...
						std::vector<uint_fast8_t> cbArray, std::vector<uint_fast8_t> crArray)
{
	...
	*/
	...
	for (size_t i = 0; i < img_res; ++i) {
		...
		out.at(i * 3 + 2)	=	(unsigned char)(1.000*Y + 0.000*(Cb-128) + 1.400*(Cr-128));
		out.at(i * 3 + 1)	=	(unsigned char)(1.000*Y - 0.343*(Cb-128) - 0.711*(Cr-128));
		out.at(i * 3)		=	(unsigned char)(1.000*Y + 1.765*(Cb-128) - 0.000*(Cr-128));
	}
